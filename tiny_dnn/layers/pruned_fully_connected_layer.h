...
class pruned_fully_connected_layer : public layer {
 public:
  ...
  void forward_propagation(const std::vector<tensor_t *> &in_data,
    ...
    set_mask(weights);
    // copy constructor
    masked_weights_ = new vec_t(weights);
    for (size_t = 0; k < params_.in_size_ * params_.out_size_; k++)
    {
      masked_weights_[k] = weights[k] * mask_[k];
    }

    std::vector<tensor_t *> maksed_in_data;
    maksed_in_data.push_back(in_data[0]);
    maksed_in_data.push_back(masked_weights_);
    if (params_.has_bias_)
      maksed_in_data.push_back(in_data[2]);

    // forward fully connected op context
    fwd_ctx_.set_in_out(maksed_in_data, out_data);
  ...
  void back_propagation(const std::vector<tensor_t *> &in_data,
    ...
    auto dW = *out_grad[1];
    for (int k = 0; k < params_.in_size_ * params_.out_size_; k++)
    {
      dW[k] *= mask_[k];
    }
  }
 ...
 protected:
  void set_params(const size_t in_size, const size_t out_size, bool has_bias, float_t pruning_percentage) {
    params_.in_size_  = in_size;
    params_.out_size_ = out_size;
    params_.has_bias_ = has_bias;
    params_.pruning_percentage_ = pruning_percentage;
  }
  ...
  void set_mask(const vec_t &weights) {
    ...
    std::partial_sort(indexed_weights.begin(), indexed_weights.begin() + std::floor(pruning_percentage_ * num_weights),
                      indexed_weights.end());
 ...
 private:
  ...
  std::shared_ptr<vec_t> masked_weights_;

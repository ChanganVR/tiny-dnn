...
void recognize_single_image(const std::string &dictionary, const std::string &src_filename) {
...
void evaluate_model(const std::string &dictionary, const std::string &data_dir_path) {
...
int main(int argc, char **argv) {
    if (argc != 4) {
        std::cout << "Usage: " << argv[0] << "path to weights file"
                  << "-S for single image"
                  << "-V for evaluation on validation set"
                  << "the path to image or folder";
        return 0;
    }
    if (std::string(argv[2]) == "-S")
        recognize_single_image(argv[1], argv[3]);
    else if (std::string(argv[2]) == "-V")
        evaluate_model(argv[1], argv[3]);
    else
